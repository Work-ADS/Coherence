...
import { MenuComponent, MenuItemComponent, MenuDividerComponent } from '@coherence/ui';
    ...
    >
      ...
      <div slot="code-tab">
        ...
        <afi-component-playground [code]="codeSnippet()">
          <div slot="controls" class="space-y-space-4">
            <fieldset>
              <legend class="font-medium text-canvas-fg mb-space-1 text-body-sm">Placement</legend>
              @for (p of placements; track p) {
                <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                  <input type="radio" name="placement" [value]="p" [checked]="placement() === p" (change)="placement.set(p)" class="accent-action" />
                  {{ p }}
                </label>
              }
            </fieldset>
            ...
            <fieldset>
              ...
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                <input type="checkbox" [checked]="showShortcuts()" (change)="showShortcuts.set(!showShortcuts())" class="accent-action" />
                Mostrar shortcuts
              </label>
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                <input type="checkbox" [checked]="showDanger()" (change)="showDanger.set(!showDanger())" class="accent-action" />
                Incluir item danger
              </label>
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                <input type="checkbox" [checked]="showDisabled()" (change)="showDisabled.set(!showDisabled())" class="accent-action" />
                Item deshabilitado
              </label>
            </fieldset>
          </div>
          ...
          <div slot="preview" class="flex items-center justify-center py-space-12">
            <afi-menu [open]="menuOpen()" [placement]="placement()" (openChange)="menuOpen.set($event)">
              <button
                type="button"
              ...
              <afi-menu-item label="Crear carpeta" iconStart="folder-plus" [shortcut]="showShortcuts() ? '⌘ N' : null" (clicked)="menuOpen.set(false)" />
              <afi-menu-item label="Renombrar" iconStart="edit" [shortcut]="showShortcuts() ? '⌘ R' : null" (clicked)="menuOpen.set(false)" />
              <afi-menu-item label="Duplicar" iconStart="copy" [disabled]="showDisabled()" (clicked)="menuOpen.set(false)" />
              <afi-menu-item label="Compartir" iconStart="share" (clicked)="menuOpen.set(false)" />
              @if (showDanger()) {
                <afi-menu-divider />
                <afi-menu-item label="Eliminar" iconStart="trash" variant="danger" [shortcut]="showShortcuts() ? 'Supr' : null" (clicked)="menuOpen.set(false)" />
              }
            </afi-menu>
          </div>
        ...
        <section>
          ...
          <p class="text-body-md text-neutral-600 mb-space-8">
            Menu con trigger Button ghost icon-only (⋮). Menu dentro de NavSection con trailing action.
            Menu en fila de tabla con acciones por registro.
          </p>
        ...
        <section>
          ...
          <h3 id="menu-entradas" class="text-body-md font-medium text-canvas-fg mb-space-3">Menu — Entradas</h3>
          ...
          <h3 id="menu-salidas" class="text-body-md font-medium text-canvas-fg mb-space-3">Menu — Salidas</h3>
          <div class="overflow-x-auto rounded-lg border border-border-hairline mb-space-8">
            <table class="w-full text-body-sm">
              <thead>
                <tr class="bg-neutral-50 border-b border-border-hairline">
          ...
          <h3 id="menuitem-entradas" class="text-body-md font-medium text-canvas-fg mb-space-3">MenuItem — Entradas</h3>
          <div class="overflow-x-auto rounded-lg border border-border-hairline mb-space-8">
            <table class="w-full text-body-sm">
              <thead>
                <tr class="bg-neutral-50 border-b border-border-hairline">
          ...
          <h3 id="menuitem-salidas" class="text-body-md font-medium text-canvas-fg mb-space-3">MenuItem — Salidas</h3>
            ...
            <table class="w-full text-body-sm">
              ...
              <tbody>
                <tr class="border-b border-border-hairline last:border-b-0">
                  ...
                  <td class="px-space-4 py-space-3 font-mono">{{ '{ event: MouseEvent | KeyboardEvent }' }}</td>
      ...
      <div slot="design-tab">
        ...
        <section>
          ...
          <ul class="list-disc pl-space-6 text-body-md text-neutral-600 space-y-space-2 mb-space-8">
            <li>Panel usa <code class="font-mono text-action-700">role="menu"</code> con <code class="font-mono text-action-700">aria-orientation="vertical"</code>.</li>
            ...
            <li>Trigger (responsabilidad del consumidor): <code class="font-mono text-action-700">aria-haspopup="menu"</code> + <code class="font-mono text-action-700">aria-expanded</code>.</li>
            <li>Items disabled: <code class="font-mono text-action-700">aria-disabled</code>, no reciben foco.</li>
          </ul>
          ...
          <div class="space-y-space-3 mb-space-8">
            ...
            <div class="flex items-center gap-space-3">
              ...
              <span class="text-body-md text-neutral-600">Cierra el menú, devuelve foco al trigger</span>
            </div>
            <div class="flex items-center gap-space-3">
        ...
        <section>
          ...
          <ul class="list-disc pl-space-6 text-body-md text-neutral-600 space-y-space-2 mb-space-6">
            <li>Entrada panel: <code class="font-mono">opacity 0→1 + translateY(-4px→0)</code>, <code class="font-mono">duration-fast</code>.</li>
            <li>Hover item: <code class="font-mono">bg transition duration-fast</code>.</li>
            <li>Hover lean-in ícono: <code class="font-mono">translateX(2px) 120ms ease-out</code>.</li>
          ...
          <div class="space-y-space-4">
            <div class="grid grid-cols-2 gap-space-4">
              <div class="p-space-4 border border-system-success rounded-md">
                ...
                <p class="text-body-sm text-neutral-600">Acciones destructivas (Eliminar) al final, separadas por divider, con variante danger.</p>
              </div>
              <div class="p-space-4 border border-system-error rounded-md">
                ...
                <p class="text-body-sm text-neutral-600">Mezclar acciones destructivas con acciones normales sin separación visual.</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-space-4">
              <div class="p-space-4 border border-system-success rounded-md">
                <p class="text-body-sm font-medium text-system-success mb-space-2">Correcto</p>
                <p class="text-body-sm text-neutral-600">Máximo 8 items por menú. Si necesita más, agrupe o use un Drawer.</p>
              </div>
              <div class="p-space-4 border border-system-error rounded-md">
                <p class="text-body-sm font-medium text-system-error mb-space-2">Incorrecto</p>
                <p class="text-body-sm text-neutral-600">Usar Menu para selección de valores — eso es un Select.</p>
              </div>
            </div>
          </div>
        </section>
...
export class MenuPage {
  ...
  readonly menuInputs = [
    ...
    { name: 'placement', type: 'MenuPlacement', default: "'bottom-start'", notes: 'Posición relativa al trigger' },
    { name: 'ariaLabel', type: 'string', default: "'Menú contextual'", notes: 'Label accesible del menú' },
  ];

  readonly menuOutputs = [
    { name: 'openChange', payload: 'boolean', notes: 'Emitido al abrir/cerrar' },
    { name: 'dismissed', payload: "'escape' | 'click-outside' | 'item-select'", notes: 'Razón del cierre' },
  ];

  readonly menuItemInputs = [
    { name: 'label', type: 'string', default: '(required)', notes: 'Texto del item' },
    { name: 'iconStart', type: 'string | null', default: 'null', notes: 'Nombre de ícono (display only)' },
    { name: 'shortcut', type: 'string | null', default: 'null', notes: 'Hint de atajo (display only)' },
    { name: 'variant', type: "'default' | 'danger'", default: "'default'", notes: 'Variante visual' },
    { name: 'disabled', type: 'boolean', default: 'false', notes: 'Desactiva el item' },
    { name: 'ariaLabel', type: 'string | null', default: 'null', notes: 'Override de label para SR' },
  ];
}
