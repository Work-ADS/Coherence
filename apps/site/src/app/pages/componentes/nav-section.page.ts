...
import { NavSectionComponent, NavItemComponent, SidebarComponent } from '@coherence/ui';
    ...
    >
      ...
      <div slot="code-tab">
        ...
        <afi-component-playground [code]="codeSnippet()">
          <div slot="controls" class="space-y-space-4">
            <fieldset>
              ...
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                <input type="checkbox" [checked]="showTreeLines()" (change)="showTreeLines.set(!showTreeLines())" class="accent-action" />
                showTreeLines
              </label>
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
                <input type="checkbox" [checked]="defaultExpanded()" (change)="defaultExpanded.set(!defaultExpanded())" class="accent-action" />
                defaultExpanded
              </label>
              <label class="flex items-center gap-2 py-0.5 cursor-pointer text-body-sm">
          ...
          <!-- Live preview -->
          <div slot="preview" class="h-[320px] flex border border-border-hairline rounded-lg overflow-hidden">
            <afi-sidebar mode="static" ariaLabel="Navegación de ejemplo">
              <span slot="top" class="text-body-md font-medium text-canvas-fg truncate">Coherence</span>
        ...
        <section>
          ...
          <p class="text-body-md text-neutral-600 mb-space-8">
            NavSection dentro de Sidebar con trailing action (+) para crear subcarpetas.
            NavSection anidadas para jerarquías de 2 niveles.
            NavSection con auto-expand cuando la ruta activa coincide con un hijo.
          </p>
      ...
      <div slot="design-tab">
        ...
        <section>
          ...
          <ul class="list-disc pl-space-6 text-body-md text-neutral-600 space-y-space-2 mb-space-8">
            <li>Fila padre usa <code class="font-mono text-action-700">aria-expanded</code> y <code class="font-mono text-action-700">aria-controls</code> apuntando al contenedor de hijos.</li>
            <li>Contenedor de hijos usa <code class="font-mono text-action-700">role="group"</code> con <code class="font-mono text-action-700">aria-label</code> del label de la sección.</li>
            <li><code class="font-mono text-action-700">aria-current="page"</code> solo en el NavItem hoja activo — nunca en la fila padre.</li>
            <li>Líneas de árbol, trail y marker son <code class="font-mono text-action-700">aria-hidden="true"</code> — puramente visuales.</li>
            <li>Trailing action debe incluir contexto de sección en su ariaLabel (ej: "Crear carpeta en Richard HQ").</li>
          </ul>
          ...
          <div class="space-y-space-3 mb-space-8">
            ...
            <div class="flex items-center gap-space-3">
              ...
              <span class="text-body-md text-neutral-600">Expande (si colapsado); mueve foco al primer hijo</span>
            </div>
            <div class="flex items-center gap-space-3">
              <kbd class="px-2 py-1 bg-neutral-100 border border-border-hairline rounded text-body-sm font-mono">←</kbd>
              <span class="text-body-md text-neutral-600">Colapsa (si expandido); mueve foco al padre</span>
            </div>
            <div class="flex items-center gap-space-3">
              <kbd class="px-2 py-1 bg-neutral-100 border border-border-hairline rounded text-body-sm font-mono">↑ ↓</kbd>
        ...
        <section>
          ...
          <ul class="list-disc pl-space-6 text-body-md text-neutral-600 space-y-space-2 mb-space-6">
            <li>Expand/collapse: <code class="font-mono">grid-template-rows 0fr→1fr</code>, <code class="font-mono">duration-200 ease-out</code>.</li>
            <li>Chevron: rotación <code class="font-mono">0→90deg</code>, <code class="font-mono">duration-fast</code>.</li>
            <li>Trail de hover: <code class="font-mono">background 120ms ease-out</code> en la línea guía.</li>
            <li>Marker: <code class="font-mono">top 200ms ease-out, opacity 120ms ease-out</code>.</li>
            <li>Reduced motion: todo colapsa a instantáneo vía <code class="font-mono">prefers-reduced-motion</code>.</li>
          </ul>
        </section>
          ...
          <div class="space-y-space-4">
            <div class="grid grid-cols-2 gap-space-4">
              <div class="p-space-4 border border-system-success rounded-md">
                ...
                <p class="text-body-sm text-neutral-600">Máximo 2 niveles de anidación. Más niveles dificultan la navegación.</p>
              </div>
              <div class="p-space-4 border border-system-error rounded-md">
                ...
                <p class="text-body-sm text-neutral-600">Tres o más niveles de NavSection anidados — refactore la IA de navegación.</p>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-space-4">
              <div class="p-space-4 border border-system-success rounded-md">
                <p class="text-body-sm font-medium text-system-success mb-space-2">Correcto</p>
                <p class="text-body-sm text-neutral-600">Sección auto-expande cuando la ruta activa coincide con un hijo.</p>
              </div>
              <div class="p-space-4 border border-system-error rounded-md">
                <p class="text-body-sm font-medium text-system-error mb-space-2">Incorrecto</p>
                <p class="text-body-sm text-neutral-600">Todas las secciones abiertas por defecto — el usuario pierde contexto visual.</p>
              </div>
            </div>
          </div>
        </section>
...
export class NavSectionPage {
  ...
  readonly apiInputs = [
    ...
    { name: 'expanded', type: 'boolean | null', default: 'null', notes: 'Override manual. null = auto mode' },
    { name: 'defaultExpanded', type: 'boolean', default: 'false', notes: 'Estado inicial cuando expanded es null' },
    { name: 'disabled', type: 'boolean', default: 'false', notes: 'Desactiva la fila padre' },
    { name: 'ariaLabel', type: 'string | null', default: 'null', notes: 'Override de "Sección: {label}"' },
    { name: 'showTreeLines', type: 'boolean', default: 'true', notes: 'Muestra/oculta líneas de árbol' },
  ];

  readonly apiOutputs = [
    { name: 'expandedChange', payload: 'boolean', notes: 'Emitido al expandir/colapsar' },
    { name: 'parentClicked', payload: '{ event: MouseEvent }', notes: 'Emitido al hacer clic en la fila padre' },
  ];
}
