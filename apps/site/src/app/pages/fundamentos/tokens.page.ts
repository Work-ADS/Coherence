  ...
  template: `
    <div class="max-w-[920px] mx-auto px-space-10 py-space-12">
      ...
      <p class="text-body-md text-neutral-500 max-w-[640px] mb-space-10">
        Tres capas — primitivo, semántico y marca — gobiernan cada valor visual en Coherence.
        JSON es la fuente de verdad; CSS custom properties son artefactos generados, nunca escritos a mano.
      </p>
      ...
      <section class="mb-space-12">
        <h2 id="arquitectura-de-tres-capas" class="text-section text-canvas-fg mb-space-4">Arquitectura de tres capas</h2>
        <div class="flex flex-col md:flex-row gap-space-4 max-w-[720px]">
          @for (layer of layers; track layer.name) {
            <div class="flex-1 p-space-6 border rounded-md"
                 [class]="layer.active ? 'border-action-500 bg-surface-quiet' : 'border-border-hairline'">
      ...
      <!-- Token pipeline -->
      <section class="mb-space-12">
        <h2 id="pipeline" class="text-section text-canvas-fg mb-space-4">Pipeline de generación</h2>
        <p class="text-body-md text-neutral-600 max-w-[640px] mb-space-6">
          Style Dictionary transforma los archivos JSON en CSS custom properties. El build se ejecuta
          con <code class="font-mono text-body-sm">node libs/tokens/build.mjs</code>.
        </p>
        <div class="flex flex-col gap-space-2 max-w-[720px]">
          @for (step of pipeline; track step.label) {
            <div class="flex items-center gap-space-4 p-space-3 bg-surface-quiet rounded-md">
              <span class="text-body-sm-600 text-action-700 shrink-0 w-space-6 text-center">{{ step.num }}</span>
              <div>
          ...
          <table class="w-full text-body-sm">
            <thead>
              <tr class="border-b border-border-hairline text-left">
                ...
                <th class="py-space-3 pr-space-4 text-body-sm-600 text-canvas-fg">Ejemplo de token</th>
      ...
      <!-- Brand manifest -->
      <section class="mb-space-12">
        <h2 id="manifiesto-de-marca" class="text-section text-canvas-fg mb-space-4">Manifiesto de marca</h2>
        <p class="text-body-md text-neutral-600 max-w-[640px] mb-space-6">
      ...
      <section class="mb-space-12">
        <h2 id="resumen-de-categorias" class="text-section text-canvas-fg mb-space-4">Resumen de categorías</h2>
...
export class TokensPage {
  readonly layers = [
    { name: 'Primitivo', desc: 'Valores crudos: hex, px, ms. Sin semántica.', example: '--color-blue-500: hsl(220, 65%, 38%)', active: false },
    { name: 'Semántico', desc: 'Nombre por intención. Lo que usa el componente.', example: '--color-action-500: var(--color-blue-500)', active: true },
    { name: 'Marca', desc: 'Override de marca. Mínimo, solo lo que cambia.', example: '--color-action-500: hsl(190, 70%, 35%)', active: false },
  ];
  ...
  readonly pipeline = [
    ...
    { num: '4', label: 'Tailwind theme.extend', path: 'tailwind.config.js → tokens a utility classes' },
  ];
