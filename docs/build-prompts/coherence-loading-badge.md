...
2. `docs/accessibility.md` — Toast section (LoadingOverlay's a11y model is closest to Toast's `aria-busy` + live region); Badge's only rule is "color is never the only cue"
3. `docs/component-skill.md`
4. `docs/token-skill.md` — System bucket (status colors for Badge)
5. `docs/plan.md` — **Motion context rule (LOCKED 2026-04-16)** — decorative motion lives in DS site, consumer apps stay spare. LoadingOverlay must ship two variants accordingly.
6. `docs/plan.md` — motion.dev multi-state-badge reference
