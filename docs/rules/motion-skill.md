...
**Currently used in** — the IA-sitemap thumbnail on the blog/methodology landing ([blog.landing.scss](../../apps/site/src/app/pages/blog/blog.landing.scss), `.thumb-ia`): five stage groups, two connector draw-ins and a highlight beat, all on one shared pass. Its `@media (prefers-reduced-motion: reduce)` block is the reference implementation — it lists every animated selector and drops them to `none`, leaving the full static map.
