...
- **Layout deviation, same as Brief F:** the `+ Añadir desinversión` and `← Volver al listado` buttons live in body-level toolbars rather than the page-header's `slot="actions"`. Multi-level ng-content projection through the shared shell strips the slot attribute. Same affordance, slightly different position. Refactor to `ngTemplateOutlet` actions slot on the shell would lift this constraint for all three Objetivos pages.
