...
- **Deviation from original brief layout (RESOLVED 2026-05-26):** the `+ Añadir inversión futura` button was originally specced as `slot="actions"` inside the page-header. Multi-level ng-content projection through the shared shell briefly dropped the slot attribute, so the button lived in a `.if-toolbar` above the table. **Brief 0 (shell refactor) resolved this**: the shared shell no longer wraps `<afi-page-header>`, so the page now renders its own header inline and the `+ Añadir` CTA lives in the proper `slot="actions"`. The `.if-toolbar` div + SCSS rule have been deleted. See [`docs/session-briefs/2026-05-26-awp-chore-shell-refactor-header-slot.md`](2026-05-26-awp-chore-shell-refactor-header-slot.md).
